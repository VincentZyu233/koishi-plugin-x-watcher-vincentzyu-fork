...
- 上游分叉后的 3 个提交单独列出，**没有直接合并或 cherry-pick**；硬取消功能已手工适配并纳入 `0.4.0-beta.7+20260916`，详见下节。
- 历史版本的默认值、依赖和配置可能已被后续版本替换；当前使用方式以 [README](./readme.md) 为准。

## 🚀 本仓库独立更新

### 🏷️ 0.4.0-beta.8 — 2026-09-16

- 补齐所有指令在帮助中的完整别名，如 `xhe、xhelp`、`xre、xrecent`；图片帮助、文字帮助和指令详细说明共用定义，以顿号分隔。

### ✨ 0.4.0-beta.7+20260916 — 2026-09-16

- 新增帮助指令别名 `xhelp`，与 `xhe` 共用同一帮助入口和文案。
- 调细 `xlist`、`xrecent` 卡片标题，以及最近动态卡片中的用户名和动态类型标签。
- README 命令说明改为表格，补充说明按引用段落整理；文末直接展示订阅列表、最新动态、最近动态和帮助四张预览图。

#### 🖥️ 独立订阅管理页面与带二次确认的硬取消
